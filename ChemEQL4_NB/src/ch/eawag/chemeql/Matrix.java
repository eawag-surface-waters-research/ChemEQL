...
// -----------------------------------------------------------------------------
	...
	void datInput(File inputFile) throws IOException
	{
		...
		do
		{
			...
			catch (NumberFormatException ex) {} // just skip species[totSpec++] = z;
...
//			&& Tokenizer.stringRealOK(s) && myRead.notEOF());
		...
		if (adsorption()) /*pr¸fe, ob auf dieser Zeile Eingaben f¸r Adsorption*/
		{
			...
			{
				try {area[a] = Double.parseDouble(myRead.nextItem());}
				catch (NumberFormatException ex)
				{
					throw new DataFormatException(
			...
			{
				try {surfaceSites[a] = Double.parseDouble(myRead.nextItem());}
				catch (NumberFormatException ex)
				{
					throw new DataFormatException(
			...
			{
				try {solidConc[a] = Double.parseDouble(myRead.nextItem());}
				catch (NumberFormatException ex)
				{
					throw new DataFormatException(
			...
			try {ionicStr = Double.parseDouble(myRead.nextItem());}	/*ionicStr*/
			catch (NumberFormatException ex)
			{
				throw new DataFormatException(
			...
			if (model != ModelEnum.DIFFUSE_GTL)
			{
				...
				{
					try {innerCap[a] = Double.parseDouble(myRead.nextItem());}
					catch (NumberFormatException ex)
					{
						throw new DataFormatException(
							"Error reading adsorption input: The number of surface sites is not a number");
			...
			if (model == ModelEnum.TRIPLE_L)
			{
				...
				{
					try {outerCap[a] = Double.parseDouble(myRead.nextItem());}
					catch (NumberFormatException ex)
					{
						throw new DataFormatException(
...
// -----------------------------------------------------------------------------
	...
	boolean recalculate(int replaceCompNo, int spLibSpecNo)
	{
		...
		for (int a=0; a < spLib.libTotComp; a++)
			if (spLib.libSpecMat[spLibSpecNo][a] != 0 && a != replaceCompNo)		/*untersuche, ob die ausgew‰hlten Komponenten der Bibliothek*/
			{																	/*in der Matrix auch vorkommen*/
				boolean ok = false;
				for (int b=0; b < totComp; b++)
...
// -----------------------------------------------------------------------------
	...
	void runProc()
	{
		...
		if (main.pHrange && !main.adsRange && !main.doKinetik && !main.dopXpYdiagram)
		{	/*pH=constant or pH-range*/
	...
	private void calcKinetik()	/*reactionOrder, reactionRate, kinComp, timeInc, rateConst */
	{
		...
		while (t <= kinTimeEnd)
		{
			...
			if (reactionOrder == 21)		 /*2ndOrder:		C+D -> P + ...		-dC/dt = k*C*D*/
			{
				if (Co == Db)
					Db = Db + Db * 1e-10;
				Ct = Co - coeffC * (Co * Db * (Math.exp((Co - Db) * rateConst * kinTimeInc) - 1) / (coeffD * Co * Math.exp((Co - Db) * rateConst * kinTimeInc) - coeffC * Db));
			...
			if (reactionOrder == 22)		 /*1st Order in C:		cC -> pP,  autocatalysis   -dC/dt = k*C*P,*/
			{
				if ((Po <= 0))
					Po = 1e-20;
				Ct = 1 / coeffC * (Co - (Co * Po * (coeffP * Math.exp(rateConst * kinTimeInc * (Co + Po)) - coeffC)) / (coeffC * Co + coeffP * Po * Math.exp(rateConst * kinTimeInc * (Co + Po))));
			}
...
//		}
	...
	private int searchDominatingSpecies(boolean[] gases)
	{
		...
		for (int a=0; a < pXpYspecs; a++)	/*suche ob ein Gas*/
			if (gases[a] && speConc[a]>=1 && speConc[a]>dominatingSpecConc)
			{	/*falls ein Gas und Partialdruck >=1 atm und conc > als beim letzten*/
...
/*----------------------------------*/
		...
		while (-myLogConcY < pYrangeEnd)
		{
			...
			{									/*der n‰chste Punkt in HORIZONTALER Richtung*/
				...
				if (oldSpec != newSpec)		/*falls nicht mehr dieselbe Spezies dominiert...*/
				{
					...
					do
					{
						if (	(!isAGas && (Math.abs(speConc[oldSpec]-speConc[newSpec]) < (speConc[newSpec]/1000)))
							...
							|| (!isAGas && pXpYgases[oldSpec] && speConc[oldSpec] > 0.999))
						{						/*wenn die beiden Konz nur 0.1% auseinander..*/
							result = true;
						...
						{		/*versuchen, einen besseren Wert f¸r concEstim[xComp] zu erhalten*/
							if (!isAGas)
							{
...
//	/**** jetzt in VERTIKALER Richtung, nach oben, falls eine horizontale Trennungslinie besteht ****/
				...
				if (oldSpec != newVSpec)	/*falls nicht mehr dasselbe Spezies dominiert...*/
				{
					...
					do
					{
						...
						{		/*versuchen, einen besseren Wert f¸r concEstim[yComp] zu erhalten*/
							if (!isAGas)
							{
	...
	{
		if (noOfCheckPrecip == 0) return false; // in case precipitation is not possible
			...
			if (main.isCalcActivCoeff(ActivityEnum.DAVIES))
			{
				for (int a=0; a < totSpec; a++)
					if (species[a].name.equals("H+") || species[a].name.equals("e-"))
						...
						logfSpec[a] = -activA * zSpec[a] * zSpec[a] * (Math.sqrt(ionicStr) / (1 + Math.sqrt(ionicStr)) - 0.2 * ionicStr);
				for (int b=0; b < totComp; b++)
					if (components[b].isModeSolidPhaseOrCheckPrecip()
					|| components[b].mode.isAdsorbentX()
					|| components[b].isHPlusOrEMinus())
						...
						logfComp[b] = -activA * zComp[b] * zComp[b] * (Math.sqrt(ionicStr) / (1 + Math.sqrt(ionicStr)) - 0.2 * ionicStr);
			}
	...
	private void modifyDerivatives(int c, double[] psi)
	{
		...
		if (model == ModelEnum.CONSTANT_CAP)
			...
			jacob[chargeNo1[c]][chargeNo1[c]] += + B3 * B2 * Math.sqrt(ionicStr) * MyTools.myCosh(B3 * psi[0]) * B4 / compConc[chargeNo1[c]] * factor;
		...
		else if (model == ModelEnum.STERN_L)
			{
				...
				jacob[chargeNo2[c]][chargeNo2[c]] += + (innerCap[c] + B3 * B2 * Math.sqrt(ionicStr) * MyTools.myCosh(B3 * psi[1])) * B4 / compConc[chargeNo2[c]] * factor;
			}
		...
		else if (model == ModelEnum.TRIPLE_L)
			{
				...
				jacob[chargeNo3[c]][chargeNo3[c]] = (outerCap[c] + B3 * B2 * Math.sqrt(ionicStr) * MyTools.myCosh(B3 * psi[2])) * B4 / compConc[chargeNo3[c]] * factor;
	...
	private void calculation()
	{
		...
		do
		{
			...
			for (int b=0; b < totComp; b++)
			{
				...
				for (int a=0; a < totSpec; a++)
				{																					/* Total conc. of one component (all species added)*/
...
// --- TableModel for components in file info window ---------------------------
	...
	class ComponentsTableModel extends AbstractTableModel
	{
		...
		public Object getValueAt(int row, int col)
		{
			switch (row)
			{
				case 0: return components[col].name;
				case 1: return components[col].mode;
				case 2: return new Double(multiConcMatrix[0][col]);
				default: return null;
			}
		}
		...
		public void setValueAt(Object val, int row, int col)
		{
			switch (row)
			{
				case 0:	// change component name
					if (components[col].name.equals((String)val)) return;
					components[col].name = (String)val;
					break;
				case 1:	// change component mode
					Mode m = (Mode)val;
					if (components[col].mode == m) return;
				...
				case 2:	// change component concentration
					if (val == null) return;		// number format error
					double v = ((Double)val).doubleValue();
					if (multiConcMatrix[0][col] == v) return;
	...
	class AdsorptionTableModel extends AbstractTableModel
	{
		...
		public int getRowCount()
		{
			if (!adsorption()) return 1;

			int result = 5;
			if (model != ModelEnum.DIFFUSE_GTL) result++;
			if (model == ModelEnum.TRIPLE_L) result++;
			return result;
		}
		...
		public Object getValueAt(int row, int col)
		{
			if (col == 0)
				switch (row)
				{
					case 0: return "double layer model:";
					case 1: return "surface area [m2/g]:";
					case 2: return "surface sites [mol/m2]:";
					case 3: return "particle conc. [g/l]:";
					case 4: return "ionic strength [mol/l]:";
					case 5: return "inner capacitance [F/m2]:";
					case 6: return "outer capacitance [F/m2]:";
					default: return null;
				}
			else
				switch (row)
				{
					case 0: return col == 1 ? model.getName() : "";
					case 1: return new Double(area[col-1]);
					case 2: return new Double(surfaceSites[col-1]);
					case 3: return new Double(solidConc[col-1]);
					case 4: return col == 1 ? (Object)new Double(ionicStr) : "";
					case 5: return new Double(innerCap[col-1]);
					case 6: return new Double(outerCap[col-1]);
					default: return null;
				}
		}
		...
		public void setValueAt(Object val, int row, int col)
		{
			...
			switch (row)
			{
				case 1: oldV = area[col-1]; area[col-1] = v; break;
				case 2: oldV = surfaceSites[col-1]; surfaceSites[col-1] = v; break;
				case 3: oldV = solidConc[col-1]; solidConc[col-1] = v; break;
				case 4: oldV = ionicStr; ionicStr = v; break;	// assert: col==1
				case 5: oldV = innerCap[col-1]; innerCap[col-1] = v; break;
				case 6: oldV = outerCap[col-1]; outerCap[col-1] = v; break;
			}
			if (v != oldV)
				main.drawFileInfoWindow(true);
		}
	...
	class SpeciesTableModel extends AbstractTableModel
	{
		...
		public Object getValueAt(int row, int col)
		{
			switch (col)
			{
				...
				case 1: return species[row].name;
				case 2: return new Double(species[row].constant);
				case 3: return species[row].source;
				default: return null;
			}
		}
		...
		public void setValueAt(Object val, int row, int col)
		{
			if (col == 2)
			{
				if (val == null) return;		// number format error
				double v = ((Double)val).doubleValue();
				if (species[row].constant == v) return;
	...
	class ComponentsOutputTableModel extends AbstractTableModel
	{
		...
		public String getColumnName(int col)
		{
			switch (col)
			{
				...
				case 1: return "Mode";
				case 2: return "Initial Conc.";
				case 3: return "In or out of system";
				default: return null;
			}
		}
	...
	class PxPyOutputTableModel extends AbstractTableModel
	{
		...
		public String getColumnName(int col)
		{
			switch (col)
			{
				case 0: return "-log " + components[xComp].name;
				case 1: return "-log " + components[yComp].name;
				default: return "";
			}
		}
		...
		public Object getValueAt(int row, int col)
		{
			...
			switch (col)
			{
				case 0: return new Double(data.pX);
				case 1: return new Double(data.pY);
				case 2: return data.pXpYborderOldSpecies;
				case 3: return data.pXpYborderNewSpecies;
				default: return null;
			}
		}
	}
